...
db.prepare(`INSERT INTO users (id, email, name, password, role, emailVerified, createdAt, updatedAt)
...
const insertCat = db.prepare(`INSERT INTO categories (id, name, slug, description, isActive, sortOrder, createdAt, updatedAt)
VALUES (?, ?, ?, ?, 1, 0, datetime('now'), datetime('now'))`);
for (const cat of categories) {
  insertCat.run(cat.id, cat.name, cat.slug, cat.description);
}
...
const insertBrand = db.prepare(`INSERT INTO brands (id, name, slug, description, country, isActive, featured, sortOrder, createdAt, updatedAt)
VALUES (?, ?, ?, ?, ?, 1, ?, ?, datetime('now'), datetime('now'))`);
for (const brand of brands) {
  insertBrand.run(brand.id, brand.name, brand.slug, brand.description, brand.country, brand.featured, brand.sortOrder);
}
...
const insertProduct = db.prepare(`INSERT INTO products (id, name, slug, description, categoryId, price, basePrice, comparePrice, images, stock, isActive, isFeatured, brandId, material, color, availableSizes, availableColors, createdAt, updatedAt)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`);
for (const product of products) {
  insertProduct.run(
    product.id, product.name, product.slug, product.description, product.categoryId,
    product.price, product.basePrice, product.comparePrice, product.images,
    product.stock, product.isFeatured, product.brandId, product.material,
    product.color, product.availableSizes, product.availableColors
  );
}
