...
export async function middleware(request: NextRequest) {
  ...
  const createSecureResponse = (baseResponse: NextResponse) => {
    ...
    // Content Security Policy - allow YouTube videos and scripts
    response.headers.set(
      'Content-Security-Policy',
      "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://www.youtube.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: https: blob:; font-src 'self' data:; connect-src 'self' https:; frame-src 'self' https://www.youtube-nocookie.com https://www.youtube.com; frame-ancestors 'self'; form-action 'self'; base-uri 'self';"
    )
